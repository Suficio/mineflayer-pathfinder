...
module.exports = function(bot)
{
    ...
    function CSP()
    {
            ...
            bot.navigate.startPosition.rhs > bot.navigate.startPosition.g); i++)
        {
            ...
            else if (u.g > u.rhs)
            {
                ...
                for (let n = 0; n < predecessors.length; n++)
                {
                    let s = predecessors[n];
    ...
    function cBC(m, blockConditions, playerPosition)
    {
        ...
        for (let i = 0, il = blockConditions.conditions.length; i < il; i++)
        {
            ...
            const blockWorldData = bot.blockAt(blockWorldCoordinate);

            const bool = (condition.condition === 'solid' && blockWorldData.boundingBox !== 'block')||(condition.condition === 'empty' && blockWorldData.boundingBox !== 'empty');
            if (bool)
            {
    ...
    function M(startCoordinates, endCoordinates)
    {
        ...
        const startPosition = new st(startCoordinates);
        const endPosition = new st(endCoordinates);
        ...
        let start = bot.navigate.startPosition;
        const end = bot.navigate.endPosition;
        CSP();
        ...
        if (!start.p.equals(end.p))
        {
            ...
            start = function()
            {
                ...
                const successors = gMS(start.p, bot.navigate.successorConditions);

                for (let n = 0; n < successors.length; n++)
                {
                    let s = successors[n];
                    if (cS(s)) s = bot.navigate.S[s.x >>> 0][s.y >>> 0][s.z >>> 0]; else continue;

                    const v = bot.navigate.COST(start, s) + s.g;
                    if (v < min)
                    {
                        min = v;
                        d = s;
...
Vec3.Vec3.prototype.rproduct = function(direction)
{
    ...
    const vector = [Vec3(1, 1, 1), Vec3(-1, 1, 1), Vec3(-1, 1, -1), Vec3(1, 1, -1)][direction];
